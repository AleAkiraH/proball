...
# def get_list_series_availables(url):
#     response = [None]
#     response[0] = requests.get(url, verify=False)
#     return response[0]
...
# 	if(request_betsapi_com(response, url)):
...
# 		infinflated = decompress.decompress(response.read())lated += decompress.flush()
# 		return inflated

# 	return response.read()


# def request_betsapi_com(response, url):
# 	"""Tries to request the URL. Returns True if the request was successful; false otherwise.
# 	https://betsapi.com/cs/soccer/2020-12-1?skipE=1
